...
public class MyService3 extends Service {
            ...
            switch (msg.what) {
                ...
                case SendDateToServer.GETVALUE:
                    if(xuanzhi==null) {
                        ...
                        xuanzhi1=xuanzhi;
                    Intent inta = new Intent("com.example.broadcasttest3.MY_BROADCAST");//广播
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                Log.d("MyService4", "run");
                xuanzhi=intent.getStringExtra("xuanzhi");
                shuaxin=intent.getIntExtra("shuaxin",shuaxin);//缺省值
                if(xuanzhi==null){

                    new SendDateToServer(handler).SendDataToServer(Zhongduan4.id,xuanzhi1);
                }else
                    new SendDateToServer(handler).SendDataToServer(Zhongduan4.id,xuanzhi);
        ...
        Intent i = new Intent(this, MyService3.class);
