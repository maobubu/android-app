...
public class MyService5 extends Service {
            ...
            switch (msg.what) {
                ...
                case SendDateToServer.GETVALUE:
                    if(xuanzhi==null) {
                        ...
                        xuanzhi1=xuanzhi;
                    Intent inta = new Intent("com.example.broadcasttest5.MY_BROADCAST");//广播
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                Log.d("MyService6", "run");
                xuanzhi=intent.getStringExtra("xuanzhi");
                shuaxin=intent.getIntExtra("shuaxin",shuaxin);//缺省值
                if(xuanzhi==null){

                    new SendDateToServer(handler).SendDataToServer(Zhongduan6.id,xuanzhi1);
                }else
                    new SendDateToServer(handler).SendDataToServer(Zhongduan6.id,xuanzhi);
        ...
        Intent i = new Intent(this, MyService5.class);
