...
public class MyService6 extends Service {
            ...
            switch (msg.what) {
                ...
                case SendDateToServer.GETVALUE:
                    if(xuanzhi==null) {
                        ...
                        xuanzhi1=xuanzhi;
                    Intent inta = new Intent("com.example.broadcasttest6.MY_BROADCAST");//广播
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                Log.d("MyService7", "run");
                xuanzhi=intent.getStringExtra("xuanzhi");
                shuaxin=intent.getIntExtra("shuaxin",shuaxin);//缺省值
                if(xuanzhi==null){
                    new SendDateToServer(handler).SendDataToServer(Zhongduan7.id,xuanzhi1);
                }else
                    new SendDateToServer(handler).SendDataToServer(Zhongduan7.id,xuanzhi);
        ...
        Intent i = new Intent(this, MyService6.class);
