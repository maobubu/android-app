...
public class MyService2 extends Service {
            ...
            switch (msg.what) {
                case SendDateToServer.GETVALUE:
                    if(xuanzhi==null) {
                        ...
                        xuanzhi1=xuanzhi;
                    Intent inta = new Intent("com.example.broadcasttest2.MY_BROADCAST");//广播
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                Log.d("MyService3", "run");
                xuanzhi=intent.getStringExtra("xuanzhi");
                shuaxin=intent.getIntExtra("shuaxin",shuaxin);//缺省值
                if(xuanzhi==null){

                    new SendDateToServer(handler).SendDataToServer(Zhongduan3.id,xuanzhi1);
                }else
                    new SendDateToServer(handler).SendDataToServer(Zhongduan3.id,xuanzhi);
