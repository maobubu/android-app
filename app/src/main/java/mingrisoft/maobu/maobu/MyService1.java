...
public class MyService1 extends Service {
            ...
            switch (msg.what) {
                ...
                case SendDateToServer.GETVALUE:
                    if(xuanzhi==null) {
                        ...
                        xuanzhi1=xuanzhi;
                    Intent inta = new Intent("com.example.broadcasttest1.MY_BROADCAST");//广播
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                Log.d("MyService2", "run");
                xuanzhi=intent.getStringExtra("xuanzhi");
                shuaxin=intent.getIntExtra("shuaxin",shuaxin);//缺省值
                if(xuanzhi==null){
                    new SendDateToServer(handler).SendDataToServer(Zhongduan2.id,xuanzhi1);
                }else
                    new SendDateToServer(handler).SendDataToServer(Zhongduan2.id,xuanzhi);

            }

        }).start();
        AlarmManager manager = (AlarmManager) getSystemService(ALARM_SERVICE);
        int anHour = shuaxin * 1000;//刷新
