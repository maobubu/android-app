...
public class MyService9 extends Service {
            ...
            switch (msg.what) {
                ...
                case SendDateToServer.GETVALUE:
                    if(xuanzhi==null) {
                        ...
                        xuanzhi1=xuanzhi;
                    Intent inta = new Intent("com.example.broadcasttest9.MY_BROADCAST");//广播
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                Log.d("MyService10", "run");
                xuanzhi=intent.getStringExtra("xuanzhi");
                shuaxin=intent.getIntExtra("shuaxin",shuaxin);//缺省值
                if(xuanzhi==null){
                    new SendDateToServer(handler).SendDataToServer(Zhongduan10.id,xuanzhi1);
                }else
                    new SendDateToServer(handler).SendDataToServer(Zhongduan10.id,xuanzhi);
        ...
        Intent i = new Intent(this, MyService9.class);
